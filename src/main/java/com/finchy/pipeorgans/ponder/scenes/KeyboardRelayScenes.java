...
public class KeyboardRelayScenes {
    public static void keyboardRelaySetup(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        //PonderUtil.showKeybind(scene, relayTop, Pointing.DOWN, midiConfigKeybind, PonderTimings.READING_TIME-20-PonderTimings.CONTEXT_INFO_BUFFER);
        ...
        scene.addKeyframe();
        scene.overlay().showText(PonderTimings.READING_TIME)
                .text("You can specify a redstone frequency for a particular MIDI channel by Sneaking and Right-Clicking, then putting an item in the correct slot")
                .placeNearTarget()
                .pointAt(util.vector().blockSurface(keyboardRelay, Direction.WEST));
        ...
        for (int i = 0; i <= 4; i++) {
            ...
            scene.overlay().showFilterSlotInput(redstoneLinkVec, 70);
        }
        ...
        scene.addKeyframe();
        scene.overlay().showText(PonderTimings.READING_TIME)
                .text("Activate it by Right-Clicking")
                .placeNearTarget()
                .pointAt(util.vector().blockSurface(keyboardRelay, Direction.WEST));
        ...
        //Cycle through links
        //Number one
        PonderUtil.setKeyboardKey(scene, keyboardElement, 7);
        ...
        //Number two
        PonderUtil.setKeyboardKey(scene, keyboardElement, 6);
        ...
        //Number three
        PonderUtil.setKeyboardKey(scene, keyboardElement, 5);
        ...
        //Number four
        PonderUtil.setKeyboardKey(scene, keyboardElement, 4);
        ...
        //Number five
        PonderUtil.setKeyboardKey(scene, keyboardElement, 3);
        ...
        scene.world().modifyBlock(keyboardRelay,s -> s.cycle(KeyboardRelayBlock.TRANSMITTING), false);
        PonderUtil.setKeyboardKey(scene, keyboardElement, 0);
