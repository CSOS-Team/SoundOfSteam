...
public class QuadrupleExtensionBlock extends GenericExtensionBlock {
    ...
    public QuadrupleExtensionBlock(Properties pProperties) {
        ...
        registerDefaultState(defaultBlockState()
                .setValue(SHAPE, GenericWhistleProperties.QuadrupleExtensionShape.SINGLE)
    ...
    public InteractionResult onSneakWrenched(BlockState state, UseOnContext context) {
        ...
        if (context.getClickLocation().y < context.getClickedPos()
                .getY() + .25f || state.getValue(SHAPE) == GenericWhistleProperties.QuadrupleExtensionShape.SINGLE)
            return sneakWrenchedRemove(state, context);

        if (context.getClickLocation().y < context.getClickedPos()
                .getY() + .5) {
            world.setBlock(pos, state.setValue(SHAPE, GenericWhistleProperties.QuadrupleExtensionShape.SINGLE), 3);
        ...
        if (context.getClickLocation().y < context.getClickedPos()
                .getY() + 1f) {
            world.setBlock(pos, state.setValue(SHAPE, GenericWhistleProperties.QuadrupleExtensionShape.TRIPLE), 3);
            IWrenchable.playRemoveSound(world, pos);
            return InteractionResult.SUCCESS;
        }
