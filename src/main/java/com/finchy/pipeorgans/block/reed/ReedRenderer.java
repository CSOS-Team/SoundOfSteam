...
public class ReedRenderer extends SafeBlockEntityRenderer<ReedBlockEntity> {

    public ReedRenderer(BlockEntityRendererProvider.Context context) {}

    @Override
    protected void renderSafe(ReedBlockEntity be, float partialTicks, PoseStack ms, MultiBufferSource bufferSource, int light, int overlay) {
        BlockState blockState = be.getBlockState();
        if (!(blockState.getBlock() instanceof ReedBlock))
            return;

        Direction direction = blockState.getValue(ReedBlock.FACING);
        Generic.WhistleSize size = blockState.getValue(ReedBlock.SIZE);
